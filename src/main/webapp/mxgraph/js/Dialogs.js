...
function Dialog(editorUi, elt, w, h, modal, closable, onClose)
{
	...
	if (this.bg == null)
	{
		...
		mxUtils.setOpacity(this.bg, 0);     //KRISHNA
 ...
 */
function SaveDialog(editorUi,type)
{
	var row, td;
	var div = document.createElement('div');
	var table = document.createElement('table');
	...
	nameInput.setAttribute('value', paramHandler.prototype.serviceName);
	...
	row.appendChild(td);
	var td = document.createElement('td');
	var textarea = document.createElement('textarea');          //anjali
	...
	{
		if(type=='t')
		editorUi.save(nameInput.value, type,  textarea.value);          //anjali
		else
		editorUi.save(nameInput.value, type, 'null');
    	editorUi.hideDialog();
	});
...
function EditFileDialog(editorUi)
{
	...
	paramHandler.prototype.encoderSaveFlag = false;		//Ensures the connector problems doesn't interfere
	textarea.value = mxUtils.getPrettyXml(editorUi.editor.getGraphXml());
	paramHandler.prototype.encoderSaveFlag = true;		//Re-initializes connector problem variable
	...
	td.appendChild(mxUtils.button(mxResources.get('save'), mxUtils.bind(this, function()
	{
		...
		else
		{
	        ...
	        if (format == 'xml')
	    	{
	        	...
	        	xml = xml.replace(/>\s*/g, '>');  
				xml = xml.replace(/\s*</g, '<'); 
				//var xml_revised = encodeURIComponent(formatterConverter.prototype.jsontoxml(formatterConverter.prototype.xmltojson(xml)));
				
				//window.open(xml);
	        	var blobxml = new Blob([xml], {type: "text/plain;charset=" + "UTF-8"});
				saveAs(blobxml, 'Service'+ ".txt"); 
				paramHandler.prototype.encoderSaveFlag = true;		//Re-initializes connector problem variable
	    	}
	        else if (format == 'svg')
	    	{
	        ...
	        else
	        {
					...
					height < MAX_HEIGHT && height > 0)
				{
					...
					new mxXmlRequest(EXPORT_URL, 'filename=' + name + '&format=' + format +
	        			'&bg=' + bg + '&w=' + w + '&h=' + h + '&xml=' + encodeURIComponent(xml)).
