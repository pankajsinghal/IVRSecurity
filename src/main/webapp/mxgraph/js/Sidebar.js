...
Sidebar.prototype.init = function()
{
	...
	this.addImagePalette('object', mxResources.get('object'), dir + '/clipart/', '_128x128.png',        //KRISHNA
		['Start','Answer','Dialout','Patch', 'Unpatch', 'Navigation','Processing','Database', 'DigitCollection', 'Multiplay', 'Play', 'DynamicPlay',
		 'MagicParrot', 'Live', 'URL','Timer', 'Goto','FreeResources', 'Wait', 'Vxml','Validation', 'RecordDedication','Randomizer','Probability','StopRecording', 'Exit']);
	//this.addGeneralPalette(false);
	this.addArrowPalette(false);            //KRISHNA
 ...
 */
Sidebar.prototype.thumbBorder = 5;
 ...
 */
Sidebar.prototype.gearImage = STENCIL_PATH + '/object/Gear_128x128.png';

/**
 * Specifies the width of the thumbnails.
 */
Sidebar.prototype.thumbWidth = 35;
	...
	if (this.enableTooltips && this.showTooltips)
	{
		if (this.currentElt != elt)
		{
			...
			var show = mxUtils.bind(this, function()
			{
				...
				var styleval = cells[0].imageName;            //anjali
				
				if(this.tooltip.childNodes[1] == undefined)
				{
					
					label = document.createTextNode(styleval);
					label.id = "imgNameID";
					if(styleval!=null)					//krishna
					{
				...
				{	
					this.tooltip.childNodes[1].data = styleval;
				}
				...
				this.tooltip.style.height = height+ 10 + 'px';
				...
				this.graph2.model.clear();                   //anjali
				this.graph2.addCells(cells);               //anjali
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function(style, width, height, value, imgName)    //anjali
{
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	cells[0].vertex = true;
	cells[0].imageName = imgName;    //anjali
	cells[0].type = imgName;            //KRISHNA
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createEdgeTemplate = function(style, width, height, value, type)	//KRISHNA
{
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	cells[0].geometry.setTerminalPoint(new mxPoint(0, height), true);
	cells[0].geometry.setTerminalPoint(new mxPoint(width, 0), false);
	cells[0].edge = true;
	cells[0].type = type;		//KRISHNA
...
Sidebar.prototype.addImagePalette = function(id, title, prefix, postfix, items)
{
	this.addPalette(id, title, true, mxUtils.bind(this, function(content)
    {
    	for (var i = 0; i < items.length; i++)
		{
			...
			content.appendChild(this.createVertexTemplate('image;image=' + icon, 100, 100, '',items[i]));   //anjali
		}
    }));
};
...
Sidebar.prototype.addArrowPalette = function (expand) {
    this.addPalette('connector', mxResources.get('connector'), expand || true, mxUtils.bind(this, function (content) {    //KRISHNA
        //content.appendChild(this.createEdgeTemplate('edgeStyle=none;endArrow=none;', 100, 100));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=none', 100, 100));
		content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;strokeWidth=3;strokeColor=black', 100, 100, '', 'Normal'));
	    content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;strokeWidth=3;strokeColor=black', 100, 100, 'Check', 'Check', 'Dashed Line', true));
	    content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;strokeWidth=3;strokeColor=black', 100, 100, 'DB', 'DB', 'Dotted Line', true));
		content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;strokeWidth=3;strokeColor=black', 100, 100, 'DTMF', 'DTMF', 'Dotted Line', true));
		//content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;dashed=1;strokeWidth=4;strokeColor=blue', 100, 100, 'Processing', 'Processing', 'Dotted Line', true));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=horizontal;dashed=1;dashPattern=2 6;', 100, 100, 'Normal', 'Normal'));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=elbowEdgeStyle;elbow=vertical', 100, 100));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=entityRelationEdgeStyle', 100, 100));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=segmentEdgeStyle', 100, 100));
        //content.appendChild(this.createEdgeTemplate('edgeStyle=orthogonalEdgeStyle', 100, 100));
        //content.appendChild(this.createEdgeTemplate('arrow;edgeStyle=elbowEdgeStyle;elbow=horizontal', 100, 100, 'Processing', 'Processing'));
    }));
}; 