...
EditorUi = function(editor, container)
{
	...
	window.onbeforeunload = function()      //anjali
	{
		if (editor.modified)
		{
		...
		window.location.href = ui.getUrl ();
	}; 
 ...
 */
/*EditorUi.prototype.save = function(name, type, description)
{
	if (name != null)
	{
	    var xml = mxUtils.getPrettyXml(this.editor.getGraphXml());  //KRISHNA
		...
		try
		{
			...
			else
			{
				if (xml.length < MAX_REQUEST_SIZE)
				{
					...
					//array_val[1] = xml;
					//array_val[2] = type;
					//array_val[3] = description;
					new mxXmlRequest(SAVE_URL, 'filename=' + name + '&xml=' + xml + '&savetype=' + type + '&desc=' + description ).simulate(document, "_self"); //anjali
					//xmlhttp.open("GET", SAVE_URL + '/' + name + '/' + xml + '/' + type + '/' + description, true);
					//serviceSave('/' + name + '/' + type + '/' + description +'?xml='+xml)
					//serviceSave('/'+array_val)
				}
				else
				{
...
EditorUi.prototype.save = function(name, type, description)
{
    ...
    xmlhttp.onreadystatechange = function () {
        ...
        {
            returntext = xmlhttp.responseText;	//returntext=false -> valid
												//returntext=true  -> already exist
			
			var cookie_value = null;
			var cookieName = 'xmlsent' + "=";
			...
			for(var i=0;i < ca.length;i++) {
				...
				{
					cookie_value=c.substring(cookieName.length,c.length);
					break;
				}
			}
			
			if(document.cookie != "" && cookie_value != null)
					cookie_value = cookie_value.split('"').join("").split('$');
			
			if(document.cookie == "" || cookie_value == null)
			{
				if(returntext == 'false')		//IF VALID
				{
					EditorUi.prototype.saveService(saveInfo, name, type, description);
				}
				else
				{
					alert("Service name already exists. Please choose another name");
				}
			}
			else
			{
				if(cookie_value[0] == paramHandler.prototype.serviceName)
				{
					if(type == 'a' && returntext == 'true')		//USER PERFORMS "SAVE AS" ON "OPEN"
						alert("Service name already exists. Please choose another name");
					else
						EditorUi.prototype.saveService(saveInfo, name, type, description);
				}
    ...
    var url =  'duplicateServiceCheck/' + paramHandler.prototype.serviceName + '/' + paramHandler.prototype.shortCode;
    xmlhttp.open("GET", url, true);
    xmlhttp.send(null);
};

EditorUi.prototype.saveService = function(saveInfo, name, type, description)
{
	if (name != null)
	{
		var xml = mxUtils.getPrettyXml(saveInfo.editor.getGraphXml());  //KRISHNA
		...
		try
		{
			...
			else
			{
				if (xml.length < MAX_REQUEST_SIZE)
				{
					...
					//var blobxml = URL.createObjectURL(blob);
					new mxXmlRequest(SAVE_URL, 'filename=' + name + '&xml=' + xml + '&savetype=' + type + '&desc=' + description ).simulate(document, "_self"); //anjali

					saveInfo.editor.setStatus('Saving...');
				}
				else
				{
					mxUtils.alert(mxResources.get('drawingTooLarge'));
			...
			saveInfo.editor.filename = name;
			saveInfo.editor.modified = false;
		}
		catch (e)
		{
			alert(e);
			saveInfo.editor.setStatus('Error saving file');
		}
	}
};
//
//function checkServiceAlreadyExists(value){
//	//alert(value);
//    var xmlhttp;
//    var returntext = null;
//	
...
//        }
//    };
//    var url = CHECK_DUPLICATE_SERVICE +value;
//    xmlhttp.open("GET", url, true);
//    xmlhttp.send(null);
//
//
...
{
	/*if(this.editor.modified)                         //KRISHNA
	/*if(this.editor.modified)                         //KRISHNA
	{
		if(confirm("Do you want to save the graph ?"))
		{
		 this.saveFile(false,"s");
		}
		else
		window.location.href = ui.getUrl ();
	}
...
EditorUi.prototype.refresh = function()
{
	...
	this.toolbarContainer.style.top = '79px';
	this.toolbarContainer.style.height = this.toolbarHeight + 'px';
	this.sidebarContainer.style.top = '124px';
	this.sidebarContainer.style.width = effHsplitPosition + 'px';
	this.outlineContainer.style.width = effHsplitPosition + 'px';
	this.outlineContainer.style.height = effVsplitPosition + 'px';
	this.outlineContainer.style.bottom = this.footerHeight+1 + 'px';
...
EditorUi.prototype.createDivs = function()
{
	...
	this.menubarContainer.style.top = '45px';
...
{	
	var geFooterDiv = this.createDiv('geFooter');
	geFooterDiv.innerHTML = "&copy;blackNgreen, 2013";
 ...
 * Adds the label menu items to the given menu and parent.
 */
EditorUi.prototype.saveFile = function(forceDialog, type)
{
	if(paramHandler.prototype.serviceName == null)
	{
		alert('The service is either empty or not initialized. Please re-initialize the START node');
		return;
	}
	...
	if(type=='s' ||type=='a')
	{
			if (!forceDialog && this.editor.filename != null)
				{
					this.save(this.editor.getOrCreateFilename(),type);
				}
			else
				{
					this.showDialog(new SaveDialog(this,type).container, 300, 100, true, true,null,type);
				}
	}
	else {
			this.showDialog(new SaveDialog(this,type).container, 300, 200, true, true,null,type);     //anjali
		}
};
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	//bindAction(83, true, 'saveAsTemplate', true); // Anjali
    ...
    //bindAction(68, true, 'duplicate'); // Ctrl+D     //anjali
    bindAction(90, true, 'undo'); // Ctrl+Z
    bindAction(89, true, 'redo'); // Ctrl+Y
    //bindAction(88, true, 'cut'); // Ctrl+X                  //anjali
    //bindAction(67, true, 'copy'); // Ctrl+C                  //anjali
    bindAction(81, true, 'connect'); // Ctrl+Q
    //bindAction(86, true, 'paste'); // Ctrl+V             //anjali
