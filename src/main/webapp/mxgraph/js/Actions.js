...
Actions.prototype.init = function()
{
	...
	Action.prototype.editorUi = editor;
	var graph = editor.graph;

	// File actions
	this.addAction('new', function() { window.open(ui.getUrl()); });
	//this.addAction('newOpen', function() { window.open(ui.getUrl()); });
	//this.addAction('templateOpen', function() { window.open(ui.getUrl()); });

	this.addAction('newService', function() 
	{
	if(Actions.prototype.modified)                         //anjali
	{
		if(confirm("Do you want to save the graph ?"))
		{
		 ui.saveFile(false,"s")	;	 //anjali
		}
	...
	this.addAction('template', function() {    //anjali
	...
	this.addAction('save', function() { ui.saveFile(false,"s"); }, null, null, 'Ctrl+S');
	this.addAction('saveAs', function() { ui.saveFile(true,"a"); }, null, null, 'Ctrl+Shift-S');
	this.addAction('saveAsTemplate', function() { ui.saveFile(true,"t"); }, null, null, '');  //anjali
	...
	this.addAction('logout', function() { window.close(ui.getUrl()); });
...
Actions.prototype.modified = null;                         //anjali
/**
...
Action.prototype.editorUi = null;		//KRISHNA
...
function templateDialog(type)
{
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            ...
            array = returntext.split(",");
            //alert(array);
            
            display(array);
        }
    };
    var url = OPEN_TEMPLATE_GET_SERVICE_URL +"/"+type;
...
function display(array)	{
		...
		button.onclick = function(){
			this.hidden = 'hidden';
			var span = document.getElementById("message_span");
			var serviceSplit = select.options[select.selectedIndex].text.split('(');
			paramHandler.prototype.serviceName = serviceSplit[0];
			paramHandler.prototype.shortCode = serviceSplit[1].split(')')[0];
			span.hidden = false;
			getxml(select.options[select.selectedIndex].innerHTML);
		}
		...
		var label = document.createTextNode("Loading Service. Please Wait...");
		...
		EditorUi.prototype.showDialog(div, 300, 200, true, true);   //anjali
}
//KRISHNA
function getxml(value){
    ...
    xmlhttp.onreadystatechange = function () {
        ...
        {
            //returntext = xmlhttp.responseText;
        	 var doc = mxUtils.parseXml(xmlhttp.responseText);
        	 var model = new mxGraphModel();
        	 var codec = new mxCodec(doc);
        	 var children_mxCell = new Array();
        	 codec.decode(doc.documentElement, model);
        	 var children = model.getChildren(model.getChildAt(model.getRoot(), 0));
        	 Action.prototype.editorUi.graph.setSelectionCells(Action.prototype.editorUi.graph.importCells(children));
        	 EditorUi.prototype.hideDialog();
...
//            display(array);
    ...
    var url = OPEN_TEMPLATE_GET_XML_URL +'/'+value;
    xmlhttp.open("GET", url, true);
    xmlhttp.send(null);
