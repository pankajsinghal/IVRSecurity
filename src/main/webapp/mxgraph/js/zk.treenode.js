...
zk.TreeNode = function(ops, style_type) {
	var tree = ops.tree;
	this.tree = tree;
	...
	if (ops.iconCls) {
		...
		if (ops.icon) {
			...
			icon.src = zk.Tree.S;
			if(style_type == 'file')
				zk.setCss(icon, 'zk_tree_icon_music');
			else
				zk.setCss(icon, 'zk_tree_icon_folder');
		}
	}
